...
/**
 * 待处理的 AI Card 实例映射。
 * key = 目标 ID（senderId 或 conversationId）
 * SDK 通过 outbound.sendText 投递响应时，会检查这个 Map：
 * - 如果有待处理的 Card，则通过 Card 展示响应（而不是发普通文本）
 * - 如果没有，则正常发送普通文本消息
 */
const pendingAICards = new Map<string, { card: AICardInstance; config: any; log?: any }>();
...
/**
 * 组合消息防抖缓冲 Map<debounceKey, { timer, params }>
 * 钉钉发送"图文组合消息"时会拆成 picture + richText 两个独立 Stream 回调，
 * 导致同一条消息被处理两次。解决方案：
 * 缓冲 picture 类型消息 2 秒，如果期间收到同一发送者的 richText/text 消息
 * （已包含完整的图文内容），则丢弃 picture 回调。
 */
const pictureMessageBuffer = new Map<string, {
  timer: ReturnType<typeof setTimeout>;
  params: any;
}>();
const PICTURE_DEBOUNCE_MS = 2000;
...
// ============ SDK Channel Pipeline ============
// 路由和消息分发由 OpenClaw SDK 的 channel pipeline 处理，
// 确保 channel 标识（dingtalk-connector）和发送者身份（staffId）正确传递给 Gateway。
...
async function handleDingTalkMessage(params: {
  ...
  const { cfg, accountId, data, sessionWebhook, log, dingtalkConfig, _debounced } = params;
  const rt = getRuntime();
  ...
  // ===== 组合消息防抖 =====
  // 钉钉发送图文组合消息时拆成 picture + richText 两个回调，需要防抖避免重复处理
  const debounceKey = `${senderId}:${data.conversationId || 'direct'}`;

  if (!_debounced && content.messageType === 'picture') {
    // 图片消息：缓冲等待，如果后续有 richText/text 则丢弃此图片消息
    const existing = pictureMessageBuffer.get(debounceKey);
    if (existing) clearTimeout(existing.timer);

    log?.info?.(`[DingTalk][Debounce] 图片消息已缓冲 ${PICTURE_DEBOUNCE_MS}ms，等待组合消息的文本部分`);
    pictureMessageBuffer.set(debounceKey, {
      timer: setTimeout(() => {
        pictureMessageBuffer.delete(debounceKey);
        log?.info?.(`[DingTalk][Debounce] 图片消息防抖超时，作为独立图片处理`);
        handleDingTalkMessage({ ...params, _debounced: true }).catch((err: any) => {
          log?.error?.(`[DingTalk][Debounce] 处理缓冲图片消息失败: ${err.message}`);
        });
      }, PICTURE_DEBOUNCE_MS),
      params,
    });
    return; // 先不处理，等 debounce 超时或被后续消息取消
  }

  // 非图片消息：检查是否有缓冲的图片消息需要取消（richText 已包含完整图文内容）
  if (!_debounced) {
    const buffered = pictureMessageBuffer.get(debounceKey);
    if (buffered) {
      clearTimeout(buffered.timer);
      pictureMessageBuffer.delete(debounceKey);
      log?.info?.(`[DingTalk][Debounce] 取消缓冲的图片消息，${content.messageType} 消息已包含完整内容`);
    }
  }
  ...
  // ===== Session 管理 =====
  if (isNewSessionCommand(content.text)) {
  ...
  if (dingtalkConfig.separateSessionByConversation !== undefined) {
    log?.warn?.(`[DingTalk][Deprecation] 'separateSessionByConversation' 配置已废弃，会话隔离由 OpenClaw Gateway 的 session.dmScope 配置控制`);
  }
  if (dingtalkConfig.sharedMemoryAcrossConversations !== undefined) {
    log?.warn?.(`[DingTalk][Deprecation] 'sharedMemoryAcrossConversations' 配置已废弃，记忆管理由 OpenClaw Gateway 配置控制`);
  }

  // ===== SDK 路由解析 =====
  // 通过 SDK 的 channel pipeline 解析路由，确保 channel 标识和 bindings 配置正确应用
  const route = (rt as any).channel.routing.resolveAgentRoute({
    cfg,
    channel: 'dingtalk-connector',
    accountId,
    peer: { kind: isDirect ? 'direct' : 'group', id: to },
  });
  log?.info?.(`[DingTalk][Route] agentId=${route.agentId}, sessionKey=${route.sessionKey}`);

  // ===== 获取 oapi token（用于媒体后处理） =====
  ...
  // 将图片嵌入消息文本（OpenClaw AgentMediaPayload 格式）
  if (imageLocalPaths.length > 0) {
  ...
  // ===== 构建 SDK Inbound Context =====
  const fromLabel = isDirect ? `${senderName} (${senderId})` : `${groupName} - ${senderName}`;
  const extraSystemPrompt = systemPrompts.length > 0 ? systemPrompts.join('\n\n') : undefined;

  let storePath: string | undefined;
  let previousTimestamp: number | undefined;
  try {
    storePath = (rt as any).channel.session.resolveStorePath(cfg.session?.store, { agentId: route.agentId });
    previousTimestamp = (rt as any).channel.session.readSessionUpdatedAt({ storePath, sessionKey: route.sessionKey });
  } catch (err: any) {
    log?.warn?.(`[DingTalk][Session] 解析 storePath 失败: ${err.message}`);
  }

  // 格式化 envelope（包含 channel、发送者、时间戳等元数据）
  let envelopeBody = userContent;
  try {
    const envelopeOptions = (rt as any).channel.reply.resolveEnvelopeFormatOptions(cfg);
    envelopeBody = (rt as any).channel.reply.formatInboundEnvelope({
      channel: 'DingTalk',
      from: fromLabel,
    ...
    log?.warn?.(`[DingTalk][Envelope] 格式化 envelope 失败，使用原始内容: ${err.message}`);
  }

  // 🔒 关键安全修复：通过 SDK finalizeInboundContext 正确传递 channel 和 sender 身份
  // Provider='dingtalk-connector' 让 Gateway 知道消息来源是钉钉（而非 webchat）
  // SenderId=senderId 让 Gateway 获取到发送者的 staffId，用于身份验证
  const ctx = (rt as any).channel.reply.finalizeInboundContext({
    Body: envelopeBody,
    RawBody: userContent,
    ...
    Timestamp: data.createAt,
    MediaPath: imageLocalPaths.length === 1 ? imageLocalPaths[0] : undefined,
    MediaType: imageLocalPaths.length === 1 ? 'image/jpeg' : undefined,
    MediaUrl: imageLocalPaths.length === 1 ? imageLocalPaths[0] : undefined,
  ...
  // 记录 inbound session
  try {
    await (rt as any).channel.session.recordInboundSession({
      storePath,
      sessionKey: ctx.SessionKey || route.sessionKey,
      ctx,
      updateLastRoute: { sessionKey: route.mainSessionKey, channel: 'dingtalk-connector', to, accountId },
      onRecordError: (err: unknown) => {
        log?.error?.(`[DingTalk] Session 记录失败: ${String(err)}`);
      },
    });
  } catch (err: any) {
    log?.warn?.(`[DingTalk][Session] recordInboundSession 失败: ${err.message}`);
  }

  log?.info?.(`[DingTalk] Inbound: from=${senderName} senderId=${senderId} channel=dingtalk-connector`);
  ...
  if (asyncMode) {
    ...
    try {
      ...
      log?.info?.(`[DingTalk][Async] 回执已发送`);
    } catch (ackErr: any) {
      log?.warn?.(`[DingTalk][Async] 回执发送失败: ${ackErr?.message || ackErr}`);
    }
  }

  // ===== 创建 AI Card 并存入待处理 Map =====
  // SDK 的响应会通过 outbound.sendText 投递，
  // 我们在 outbound.sendText 中检查这个 Map，找到 Card 则用 Card 展示响应
  try {
    const card = await createAICard(dingtalkConfig, data, log);
    if (card) {
      pendingAICards.set(to, { card, config: dingtalkConfig, log });
      log?.info?.(`[DingTalk] AI Card 创建成功: ${card.cardInstanceId}，已存入 pendingAICards[${to}]`);
      // 设置超时自动清理，防止泄漏
      setTimeout(() => {
        if (pendingAICards.has(to)) {
          pendingAICards.delete(to);
          log?.warn?.(`[DingTalk] pendingAICards[${to}] 超时清理（5分钟）`);
        }
      }, 5 * 60 * 1000);
    }
  } catch (err: any) {
    log?.warn?.(`[DingTalk] AI Card 创建失败，降级为普通消息: ${err.message}`);
  }

  // ===== 通过 SDK Channel Pipeline 分发消息 =====
  // SDK 自动处理：路由到正确 agent、加载 session/memory、执行 agent
  // 响应通过 deliver 回调逐 chunk 推送到 AI Card 实现流式效果，
  // 最终通过 outbound.sendText 完成媒体后处理并 finalize AI Card
  let streamedContent = '';
  try {
    await (rt as any).channel.reply.dispatchReplyWithBufferedBlockDispatcher({
      ...
      dispatcherOptions: {
        ...
        deliver: async (payload: any, info?: { kind: string }) => {
          // 检查是否有待处理的 AI Card，将每个 chunk 流式推送到 Card
          const pending = pendingAICards.get(to);
          if (pending) {
            const chunkText = typeof payload === 'string'
              ? payload
              : (payload?.text || payload?.body || '');
            if (chunkText) {
              streamedContent += chunkText;
              try {
                await streamAICard(pending.card, streamedContent, false, log);
              } catch (streamErr: any) {
                log?.warn?.(`[DingTalk] AI Card streaming chunk 失败: ${streamErr.message}`);
              }
            }
          } else {
            // 无 AI Card（创建失败或已被消费），记录日志便于排查
            log?.warn?.(`[DingTalk] deliver 被调用但无 pending AI Card: kind=${info?.kind}`);
          }
        },
      },
    });
    log?.info?.(`[DingTalk] Dispatch 完成，streamedContent=${streamedContent.length} 字符`);
  } catch (err: any) {
    log?.error?.(`[DingTalk] Dispatch 失败: ${err.message}`);
    ...
    if (pending) {
      try {
        await finishAICard(pending.card, `⚠️ 响应中断: ${err.message}`, log);
      } catch {}
    } else {
...
const dingtalkPlugin = {
  ...
  configSchema: {
    schema: {
      ...
      properties: {
        ...
        sessionTimeout: { type: 'number', default: 1800000, description: '[DEPRECATED] 已由 OpenClaw Gateway session.reset 配置控制' },
        separateSessionByConversation: { type: 'boolean', default: true, description: '[DEPRECATED] 已由 OpenClaw Gateway session.dmScope 配置控制' },
        sharedMemoryAcrossConversations: { type: 'boolean', default: false, description: '[DEPRECATED] 已由 OpenClaw Gateway 配置控制' },
  ...
  outbound: {
    ...
    sendText: async (ctx: any) => {
      ...
      // ===== 检查是否有待处理的 AI Card =====
      // 从 targetStr 中提取 raw ID，用于匹配 pendingAICards Map
      const rawTargetId = targetStr.startsWith('user:')
        ? targetStr.slice(5)
        : targetStr.startsWith('group:')
          ? targetStr.slice(6)
          : targetStr;

      const pending = pendingAICards.get(rawTargetId);
      if (pending) {
        // 有 AI Card：用 Card 展示响应，不发普通文本
        pendingAICards.delete(rawTargetId);
        log?.info?.(`[DingTalk][outbound.sendText] 找到 pendingAICard[${rawTargetId}]，通过 AI Card 投递`);

        try {
          // 媒体后处理
          let processedText = text;
          const oapiToken = config.clientId ? await getOapiAccessToken(config) : null;
          if (oapiToken) {
            processedText = await processLocalImages(processedText, oapiToken, log);
          }
          const proactiveTarget: AICardTarget = targetStr.startsWith('group:')
            ? { type: 'group', openConversationId: targetStr.slice(6) }
            : { type: 'user', userId: targetStr.startsWith('user:') ? targetStr.slice(5) : targetStr };
          processedText = await processVideoMarkers(processedText, '', config, oapiToken, log, true, proactiveTarget);
          processedText = await processAudioMarkers(processedText, '', config, oapiToken, log, true, proactiveTarget);
          processedText = await processFileMarkers(processedText, '', config, oapiToken, log, true, proactiveTarget);

          const finalContent = processedText.trim() || text.trim() || '✅ 已完成';
          await finishAICard(pending.card, finalContent, log);
          log?.info?.(`[DingTalk][outbound.sendText] AI Card 已 finalize，内容长度=${finalContent.length}`);
          return { channel: 'dingtalk-connector', messageId: pending.card.cardInstanceId || 'card' };
        } catch (err: any) {
          log?.error?.(`[DingTalk][outbound.sendText] AI Card finalize 失败，降级为普通消息: ${err.message}`);
          // 降级：继续执行下面的普通文本发送
        }
      }

      // ===== 普通文本发送（无 AI Card 或 Card 失败降级） =====
    ...
    startAccount: async (ctx: any) => {
      ...
      return new Promise((resolve) => {
        if (abortSignal) {
          ...
          // 无 abortSignal 时立即返回控制对象，避免 Promise 永远挂起
          ctx.log?.warn?.(`[${account.accountId}] 未收到 abortSignal，立即返回控制对象`);
          resolve({
            stop: () => doStop('manual'),
            isHealthy: () => !stopped,
          });
