...
启用后：机器人立即发送"🫡 任务已接收，处理中..."回执 → 后台处理请求 → 处理完成后主动推送结果。适合耗时任务，避免长时间等待。

```bash
# 启用异步模式（立即回执 + 后台处理 + 主动推送）
openclaw config set channels.dingtalk-connector.asyncMode true

# 关闭异步模式（普通对话模式，有问必答）
openclaw config set channels.dingtalk-connector.asyncMode false
```

> ⚠️ 修改后需重启 Gateway：`openclaw gateway restart`
