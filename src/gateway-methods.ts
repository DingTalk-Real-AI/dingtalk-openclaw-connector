...
export function registerGatewayMethods(api: OpenClawPluginApi) {
  ...
  /**
   * 切换群聊回复模式（AI Card / 文本 / Markdown）
   *
   * 开启 text/markdown 模式后，群聊消息将使用普通文本/markdown 发送，
   * 支持机器人互相 @，但不再展示 AI Card 流式响应。
   *
   * ℹ️ 用户也可以直接在 openclaw.json 中配置：
   * ```json
   * {
   *   "channels": {
   *     "dingtalk-connector": {
   ...
   * @example 切换为 text 模式
   * ```typescript
   * await gateway.call('dingtalk-connector.setGroupReplyMode', { mode: 'text' });
   * ```
   *
   * @example 切换回 AI Card 模式
   * ```typescript
   * await gateway.call('dingtalk-connector.setGroupReplyMode', { mode: 'aicard' });
   * ```
   */
  api.registerGatewayMethod('dingtalk-connector.setGroupReplyMode', async ({ context, params, respond }) => {
    const { loadConfig, saveConfig } = await import('openclaw/plugin-sdk/config-runtime');
    const cfg = loadConfig();
    try {
      const { mode, accountId } = (params || {}) as any;
      const validModes = ['aicard', 'text', 'markdown'];

      if (!mode || !validModes.includes(mode)) {
        return respond(false, {
          error: `Invalid mode. Must be one of: ${validModes.join(', ')}`,
          currentMode: (cfg.channels?.['dingtalk-connector'] as any)?.groupReplyMode || 'aicard',
          usage: {
            mode: 'aicard | text | markdown',
            accountId: '(optional) account ID for multi-account setup',
          },
        });
      }

      // 更新配置
      const dingtalkCfg = cfg.channels?.['dingtalk-connector'] as any || {};
      const updatedCfg = {
        ...cfg,
        channels: {
          ...cfg.channels,
      ...
      const warning = mode !== 'aicard'
        ? `\u26a0\ufe0f 注意：开启 ${mode} 模式后，群聊消息只能使用 ${mode} 格式发送，不再展示 AI Card 流式响应。机器人互相 @ 功能已开启。重启 Gateway 后生效。`
        : '✅ 已恢复 AI Card 模式，群聊消息将使用 AI Card 流式响应。重启 Gateway 后生效。';
      
      log?.info?.(`[Gateway][setGroupReplyMode] 已切换群聊回复模式: ${mode}`);
      respond(true, { mode, warning });
    } catch (err: any) {
      log?.error?.(`[Gateway][setGroupReplyMode] 错误: ${err.message}`);
      respond(false, { error: err.message });
    }
  });
